...
const libspotify = require('libspotify');
...
function Spotify() {
	...
	this.spotifyApi = new SpotifyWebApi({
		...
		redirectUri : `${process.env.SPOTIFY_CALLBACK_HOST}/callback`
	});
	...
	this.session.login(process.env.SPOTIFY_USERNAME, process.env.SPOTIFY_PASSWORD);
	this.session.once('login', function(error) {
		if (error) {
			console.log("libspotify could not log in...")
			console.log(error);
		} else {
			console.log("libspotify logged in successfully!")
			self.app = express();
			...
			self.app.listen(process.env.SPOTIFY_PORT);
		}
	});
...
Spotify.prototype.formatTitle = function (str) {
	return str.replace('&', 'and').replace(/[&\/\\#,+\(\)$~%\.!^'"\;:*?\[\]<>{}]/g, '').replace(/([\uE000-\uF8FF]|\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDDFF])/g, '').toLowerCase();
}
